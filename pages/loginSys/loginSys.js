  ...
  Object.assign({}, Zan.TopTips, {
    ...
    onLoad: function (options) {     
      if(options.showLogin=='lost'){
        this.setData({showLogin:false,fromWho:'reportLost'});
      }
      else if(options.showLogin=='login'){
        this.setData({showLogin:true,fromWho:'paymentRecord'});
      }
      var that = this;
    ...
    submit: function (e) {
      ...
      if(that.data.showLogn==false){
        sendData.reportLost='true'
      }
      wx.request({
        url: app.globalData.baseUrl + '/loginSys',
        data: sendData,
